...
def xpu_sparse_decode_fp8(
    ...
    seq_lens: torch.Tensor | None = None,  # [num_decodes] 全量序列长（原生算子因果用）
) -> None:
    ...
    # Native fast path: fused window+compressed attention op (A/B validated
    # against the torch loop below; set DSV4_TORCH_ATTN=1 to force the torch
    # path). Builds per-token index tables into the two gathered buffers and
    # calls the single fused kernel instead of the per-token loop + packing.
    import os as _os
