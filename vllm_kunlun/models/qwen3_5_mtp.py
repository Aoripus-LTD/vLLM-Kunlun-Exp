...
# vllm 0.15.1 无 module-level fused_moe_make_expert_params_mapping（0.18+ 才有）；
# FusedMoE.make_expert_params_mapping 类方法签名兼容（首参 model 对应 PR 调用的 self）
fused_moe_make_expert_params_mapping = FusedMoE.make_expert_params_mapping
...
# vllm 0.15.1 无 vllm.transformers_utils.configs.qwen3_5（0.18+ 才有）；
# transformers 5.2.0 自带 Qwen3_5TextConfig / Qwen3_5MoeTextConfig（已核实）
...
class Qwen3_5MultiTokenPredictor(nn.Module):
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        # during decode. Ref: baidu/vLLM-Kunlun PR 384.
...
class Qwen3_5MTP(nn.Module, SupportsMultiModal):
    ...
    def embed_input_ids(
        ...
        *,
        ...
        inputs_embeds = self._embed_text_input_ids(
            ...
            # vllm 0.15.1 的 SupportsMultiModal._embed_text_input_ids 要求
            # handle_oov_mm_token 必填（PR 423 基于 0.18，有默认值）
            handle_oov_mm_token=False,
        )

        if multimodal_embeddings is None or len(multimodal_embeddings) == 0:
            return inputs_embeds
        # vllm 0.15.1 passes (embeds_list, is_mm_mask); a request with no
        # multimodal data yields an empty embeds_list, which the len() check
        # above cannot see (len of the tuple is always 2). Treat that as
        # text-only to avoid _require_is_multimodal(None) below.
        if (
            isinstance(multimodal_embeddings, tuple)
            and len(multimodal_embeddings[0]) == 0
        ):
            return inputs_embeds
    ...
    def get_input_embeddings(
        ...
        """0.15.1 的 EagleProposer.propose 走 get_input_embeddings 旧接口
        （vllm-kunlun eagle.py:96），PR 423 基于 0.18 只有 embed_input_ids。"""
        return self.embed_input_ids(input_ids, multimodal_embeddings)
