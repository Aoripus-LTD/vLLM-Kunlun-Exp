#!/bin/bash
# fix_eagle_positions.sh — self.positions 改 _set/_get_positions（幂等）
# 用法：容器内 bash fix_eagle_positions.sh
set -e
...
cp "$B" "$F"
$PY "$DIR/fix_eagle_positions.py" --apply "$F"
...
echo
echo "== 4. 单测 =="
$PY "$DIR/fix_eagle_positions.py" --test "$P"

echo
echo "== 5. 最终状态 =="
md5sum "$F"
grep -n "_set_positions\|_get_positions" "$F"
echo "ALL DONE"
