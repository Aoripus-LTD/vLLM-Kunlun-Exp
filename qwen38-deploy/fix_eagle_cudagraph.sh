#!/bin/bash
# fix_eagle_cudagraph.sh — 补 use_cuda_graph / cudagraph_batch_sizes 默认值（幂等）
# 用法：容器内 bash fix_eagle_cudagraph.sh
set -e
...
cp "$B" "$F"
$PY "$DIR/fix_eagle_cudagraph.py" --apply "$F"
...
echo
echo "== 4. 单测 =="
$PY "$DIR/fix_eagle_cudagraph.py" --test "$P"
