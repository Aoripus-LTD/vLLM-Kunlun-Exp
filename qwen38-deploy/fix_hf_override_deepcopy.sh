#!/bin/bash
# fix_hf_override_deepcopy.sh — hf_config_override 深拷贝（幂等）
# 用法：容器内 bash fix_hf_override_deepcopy.sh
set -e
...
cp "$B" "$F"
$PY "$DIR/fix_hf_override_deepcopy.py" --apply "$F"
...
echo
echo "== 4. 单测 =="
$PY "$DIR/fix_hf_override_deepcopy.py" --test "$P"
