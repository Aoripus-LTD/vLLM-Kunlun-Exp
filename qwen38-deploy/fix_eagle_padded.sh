#!/bin/bash
# fix_eagle_padded.sh — prepare_next_token_ids_padded 改回 5 参数 mask（幂等）
# 用法：容器内 bash fix_eagle_padded.sh
set -e
...
cp "$B" "$F"
$PY "$DIR/fix_eagle_padded.py" --apply "$F"
...
echo
echo "== 4. 单测 =="
$PY "$DIR/fix_eagle_padded.py" --test "$P"
